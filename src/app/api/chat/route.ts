...
You are CODi, a friendly AI Marketing Assistant. You provide consultation for clients running marketing campaigns for their small to medium businesses.
...
Style:
- Keep replies short and simple (2–4 sentences max).
...
export async function POST(request: NextRequest) {
  try {
    ...
    // reports: JSON structure of all reports AI can reference in conversation. Defaults to an empty JSON structure (POSSIBLY MAKE IT OPTIONAL INSTEAD?)
    const { message, userId, reports = {} } = await request.json();

/*
  Verify Marketing CSV Imports (CommonJS)
  Run with plain Node (no TS loader needed):
    node scripts/verify-marketing-imports.cjs
    node scripts/verify-marketing-imports.cjs --id <importId>
    node scripts/verify-marketing-imports.cjs --latest 5
*/
...
async function countForImport(id, fileName) {
	const counts = { type: 'mixed', expected: 0, dbCount: 0 };
...
async function main() {
	...
	const latest = parseInt((args.latest && String(args.latest)) || '0') || 0;
